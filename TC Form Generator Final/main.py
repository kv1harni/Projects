...
root.title(
    'K.V. No.1 Vadodara Student Tc System - developed by [Aayush & Omkar]')
...
# Function for generating the TC form image
def GenerateTcFormImg(student, reason):

    form_img = Image.open("./assets/formimg.jpg")
    form_img_draw = ImageDraw.Draw(form_img)
    myFont = ImageFont.truetype('./assets/Arial.ttf', 20)
    s_address = student['local_address']

    if len(s_address) > 92:
        s_address = str(s_address[:92]) + "..."

    # School Name
    form_img_draw.text((555, 115), school_name, fill=(0,0,0), font=ImageFont.truetype('./assets/Arial.ttf', 28))

    # Date of Application
    form_img_draw.text((650, 260), str(date.today()),
                       fill=(0, 0, 0), font=myFont)

    # Student Name
    form_img_draw.text(
        (650, 293), student['std_name'], fill=(0, 0, 0), font=myFont)

    # Class-Section (with year)
    form_img_draw.text(
        (650, 327), f"{student['class']}-{student['section']}  ({student['session']})", fill=(0, 0, 0), font=myFont)

    # Father's Name 33diff
    form_img_draw.text(
        (650, 360), student['f_name'], fill=(0, 0, 0), font=myFont)

    # Mother's name
    form_img_draw.text(
        (650, 393), student['m_name'], fill=(0, 0, 0), font=myFont)

    # Local Address
    form_img_draw.text((650, 433), s_address, fill=(
        0, 0, 0), font=ImageFont.truetype('./assets/Arial.ttf', 12))

    # Admission Number
    form_img_draw.text((380, 1272), str(
        student['adm_no']), fill=(0, 0, 0), font=myFont)
...
def generateFormDetails(st_details) -> dict:
    ...
    # creating a Dict that stores Data extracted from the Tkinter Tree  Object
    st_details_dict = {
        "adm_no": st_details[0],
...
# A CallBack Function that helps with calling the Opps functions in tkinter
...
# Function for file open dialong for opening the excel file
def file_open():
    global fileStatus
    global pdFileObj
    global fileName
...
def searchStudent():
    ...
    if rowFound == None:
        messagebox.showerror(
            "Error", "No Student Data for Provided Admission Number found.\nPlease Check the data file or Admission Number Provided")
        return
...
sheet_button = Button(menu_frame, text="Open File", width=40,
                      height=2, font=20, fg="white", bg="#0078d7", command=file_open)
sheet_button.place(x=0, y=0)

#
gen_button = Button(menu_frame, text="Generate Tc", width=40, height=2,
                    font=20, fg="white", bg="#0078d7", command=popUpButton, state=DISABLED)
